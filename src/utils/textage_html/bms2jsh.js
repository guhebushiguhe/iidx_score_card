...
function notes_index(idx){
	...
	mesnum = Math.floor(posnum / 100000);
    vernum = posnum % 100000;
			...
			for(cni=cn[h][n].length-1; cni>=0; cni--){
				...
				for (cnj=0; cnj<(cnz<10 ? 1:2); cnj++) {
					...
					if (cnf&1) {
						...
						objstr=imgdir+(!legacy&&!prt&&hcn ? "hh":"")+co[jpos]+dstr
						o+="<img src="+objstr+".gif"+msc+" style='top:"+
							(nbar*hs-cnp+coy)+"px;left:"+
							(jpos? jpos*(14-d)-sh*(37-d*8)+60-d*15:(sh-1)*(98-d*7))+"px;'>";
						if(stat_on==0){if(h==1)p1o++;kc[h-1][jpos]++;npos.push((n+gap)*100000+cnp+nmergin[n+gap]);}
					}
					if (cnf&2 && !legacy) {
						// obj!
						ttlcn++;
						objstr=imgdir+(!prt&&hcn ? "hh":"")+co[jpos]+dstr;
						o+="<img src="+objstr+".gif"+msc+" style='top:"+
							(nbar*hs-cnp-cnh+coy)+"px;left:"+
							(jpos? jpos*(14-d)-sh*(37-d*8)+60-d*15:(sh-1)*(98-d*7))+"px;'>";
						if(stat_on==0){if(h==1)p1o++;kc[h-1][jpos]++;npos.push((n+gap)*100000+cnp+cnh+nmergin[n+gap]-1);}
					}
				}
			}
		}
	...
	while(sft<sdd.length){
		...
		if(v2t!=2){
			...
			for(i2=0;i2<v2o.length;i2+=2){
				...
				jpos=obr[h-1][ob2];
				if(jpos==8)continue;
				if(stat_on==0){kc[h-1][jpos]++;npos.push((n+gap)*100000+Math.floor(v2h*hs/3)+nmergin[n+gap]);}
				// obj!
	...
	for(;sft<sdd.length;sft+=2,div+=2){
		...
		for(j=0;j<=ky;j++){
			...
			if(y>>j&1){
				...
				if(stat_on==0){kc[h-1][jpos]++;npos.push((n+gap)*100000+Math.floor(nbar*div*hs/len)+nmergin[n+gap]);}
